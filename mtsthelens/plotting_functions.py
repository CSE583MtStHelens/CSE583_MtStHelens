...
def _min_max_values(read_dictionary: dict = None):
    ...
    for key, df in read_dictionary.items():
        ...
        if not isinstance(df, pd.DataFrame):
            raise TypeError(
                f"Value associated with key '{key}' is not a DataFrame."
                )
    ...
    min_df = pd.DataFrame(min_values_per_row,
                          index=read_dictionary.keys()).transpose()
    max_df = pd.DataFrame(max_values_per_row,
                          index=read_dictionary.keys()).transpose()
...
def animation(read_dictionary: dict = None,
              parameter: str = None,
              colormap: str = None):
    ...
    color_min_max = [int(df_min_max['minimum'].loc[parameter]),
